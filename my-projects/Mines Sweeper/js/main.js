// CR: Very good game.
// CR: I liked to see you sent a variable to a func
// CR: and didn't change the global var directly. VERY GOOD.
// CR: some things are missing, like the number of mines.
// CR: You could very easily improve the code
// CR: so when num of cells unopened === num of mines
// CR: the game is over with a victory and the player doesn't have to mark them.
...
function addMines(level, board, currCell) {
    ...
    for (var x = 0; x < gLevel[level].MINES; x++) {
        ...
        for (var y = 0; y < addedMines.length; y++) {
            ...
            // CR: What if in the 2nd time the 'addedMines[y].i === random1 && addedMines[y].j === random2'?
            // CR: Try it: Change the MINE count for easy level to 15
            // CR: (i.e board of 16 cells and 15 mines) and see how many mines you get...
...
// CR: It gets as an argument the button element,
// CR: it then checks the innerText which is.... 'Restart'
...
function setCurrLevel(btn) {
    ...
    // CR: return gCurrLevel to where?
    // CR: The only time you call 'setCurrLevel' func is in the 'restart' func
    // CR: and there yoe never place the return value into a variant.
    return gCurrLevel;
}

// CR: You never read the localStorage and set the bestScore.
// CR: Every refresh deletes the score.
// CR: You could do it much shorter with better classes names and a loop.
...
// CR: You should have put it in a cellClicked func or rightClick func on the table.
// CR: now I cannot right click anywhere in the document to open console or inspect for example...
